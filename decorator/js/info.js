...
class HtmlClientDataDecorator extends ClientDecorator{
    async getData(){
        const data = await super.getData();
        const newData = data.map((e, i) => {
...
(async ()=>{
    ...
    // console.log(data2);
    const htmlClient = new HtmlClientDataDecorator(upperClient);
    const data3 = await htmlClient.getData();
    divContent1.innerHTML = data3.reduce((ac, e)=>{
        return ac + e.title + e.thumbnailUrl;
    },'');
    const htmlClient2 = new HtmlClientDataDecorator(client);
